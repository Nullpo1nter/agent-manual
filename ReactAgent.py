...
def create_system_message() -> dict[str, str]:
    ...
    return {
        ...
        "content": (
            ...
            "所有待创建文件都必须位于 /generated_files 目录之下。"
        ),
    }
def trim_messages(
...
def resolve_safe_path(relative_path: str) -> Path:
    ...
    if target != base_dir and base_dir not in target.parents:
        raise ValueError("不允许写入 /generated_files 目录之外的路径。")
...
def call_llm(
    ...
    request_messages = [create_system_message(), *trim_messages(messages, MAX_HISTORY_TURNS),
    build_state_message(user_goal, state, step_logs)]
    headers = {
...
def run_task_agent(
    ...
    for step_num in range (1, MAX_AGENT_STEPS):
        decision = call_llm(
            api_key,
            user_goal,
            messages,
        ...
        step_summary = decision.get("step_summary","模型提供步骤说明")
        print(f"\n[步骤{step_num}]:{action}")
        print(f"\n[步骤说明]:{step_summary}")

        #更新任务状态
        p = decision.get("relative_path")
        if isinstance(p,str) and p.strip() :
            state["relative_path"] = p
        c = decision.get("content")
